...
exports.getETHWalletTransactions = async function (req, res) {
    try {
        ...
        console.log("2 hours before BlockNumber: ", block2)
...
exports.createUserWallet = async (req, res) => {
    ...
    if (user && currency && currency.symbol == 'ETH') {
        ...
        try {
            ...
            const newWallet = await new Wallet({ userId: userId, address: ethAddress, privateKey: ehtPrivateKey, currencyId: currencyId, name: user.firstName + "'s " + network.symbol + " Wallet" });
    ...
    else if (user && currency && currency.symbol == 'BNB') {
        // Creating BNB Wallet
        try {
            ...
            // #request the latest block number
            let latest_blocknumber = await web3.eth.getBlockNumber()
            const newWallet = await new Wallet({ userId: userId, address: bnbAddress, privateKey: bnbPrivateKey, currencyId: currencyId, name: user.firstName + "'s " + network.symbol + " Wallet" });
            const newWalletSave = await newWallet.save();
            const blockchainBalance = await new BlockchainBalance({ userId: userId, currencyId: currencyId, networkId: networkId, walletId: newWalletSave._id, onChainBalance: 0, blockNumber: latest_blocknumber });
            blockchainBalance.save();
            return res.status(200).json({ success: true, message: "Wallet created successfully", wallet: { address: newWalletSave.address } });
        ...
        try {
            ...
            // #request the latest block number
            let latest_blocknumber = await web3.eth.getBlockNumber()
            const newWallet = await new Wallet({ userId: userId, address: avaxAddress, privateKey: avaxPrivateKey, currencyId: currencyId, name: user.firstName + "'s " + network.symbol + " Wallet" });
            const newWalletSave = await newWallet.save();
            const blockchainBalance = await new BlockchainBalance({ userId: userId, currencyId: currencyId, networkId: networkId, walletId: newWalletSave._id, onChainBalance: 0, blockNumber: latest_blocknumber });
            blockchainBalance.save();
            return res.status(200).json({ success: true, message: "Wallet created successfully", wallet: { address: newWalletSave.address } });
        ...
        try {
            ...
            const newWallet = await new Wallet({ userId: userId, address: trxAddress, privateKey: trxPrivateKey, currencyId: currencyId, name: user.firstName + "'s " + network.symbol + " Wallet" });
            const newWalletSave = await newWallet.save();
            const blockchainBalance = await new BlockchainBalance({ userId: userId, currencyId: currencyId, networkId: networkId, walletId: newWalletSave._id, onChainBalance: 0, blockNumber: latest_blocknumber });
            blockchainBalance.save();
            return res.status(200).json({ success: true, message: "Wallet created successfully", wallet: { address: newWalletSave.address } });
        ...
        try {
            ...
            const newWallet = await new Wallet({ userId: userId, address: btcAddress, privateKey: btcPrivateKey, currencyId: currency._id, name: user.firstName + "'s " + network.symbol + " Wallet" });
            const newWalletSave = await newWallet.save();
            const blockchainBalance = await new BlockchainBalance({ userId: userId, currencyId: currencyId, networkId: networkId, walletId: newWalletSave._id, onChainBalance: 0, blockNumber: latest_blocknumber });
            blockchainBalance.save();
            return res.status(200).json({ success: true, message: "Wallet created successfully", wallet: { address: newWalletSave.address } });
        ...
        try {
            ...
            latest_blocknumber = latest_blocknumber.data.height;
            const newWallet = await new Wallet({ userId: userId, address: bchAddress, privateKey: bchPrivateKey, currencyId: currency._id, name: user.firstName + "'s " + network.symbol + " Wallet" });
            const newWalletSave = await newWallet.save();
            const blockchainBalance = await new BlockchainBalance({ userId: userId, currencyId: currencyId, networkId: networkId, walletId: newWalletSave._id, onChainBalance: 0, blockNumber: latest_blocknumber });
            blockchainBalance.save();
            return res.status(200).json({ success: true, message: "Wallet created successfully", wallet: { address: newWalletSave.address } });
        ...
        try {
            ...
            const newWallet = await new Wallet({ userId: userId, address: ltcAddress, privateKey: ltcPrivateKey, currencyId: currency._id, name: user.firstName + "'s " + network.symbol + " Wallet" });
            const newWalletSave = await newWallet.save();
            const blockchainBalance = await new BlockchainBalance({ userId: userId, currencyId: currencyId, networkId: networkId, walletId: newWalletSave._id, onChainBalance: 0, blockNumber: latest_blocknumber });
            blockchainBalance.save();
            return res.status(200).json({ success: true, message: "Wallet created successfully", wallet: { address: newWalletSave.address } });
