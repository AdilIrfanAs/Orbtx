...
const Index = () => {
  ...
  useEffect(() => {
    if (currencyData && currencyData.length) {
      setSelectedCurrency(currencyData.find(row => row.symbol == coin));
      setCoinIndex(currencyData.findIndex(row => row.symbol == coin));
      setLoader(false)
    }
  }, [currencyData]);
        ...
        if (row.txHash) {
          ...
          if (thisCoin) {
            if (row.transactionType == 0) // Check if withdraw or deposit
              dataArr.push({
                symbol: row?.currency, coin: thisCoin?.name, amount: row?.amount, sign: '+', type: 1, category: 'Deposit',
                txHash: row.txHash, fromAddress: row.fromAddress, toAddress: row.toAddress, date: row.createdAt
              })
            else
              dataArr.push({
                symbol: row?.currency, coin: thisCoin?.name, amount: row?.amount, sign: '-', type: 1, category: 'Withdraw',
                txHash: row.txHash, fromAddress: row.fromAddress, toAddress: row.toAddress, date: row.createdAt
              })
          }
        }
          ...
          dataArr.push({
            symbol: row.toCurrency.symbol, coin: row.toCurrency.name, amount: row.convertedAmount, sign: '+', type: 2, category: 'Conversion',
            id: row._id, otherCoin: row.fromCurrency.name, otherAmount: row.fromAmount, rate: row.conversionRate, date: row.createdAt
          })
          dataArr.push({
            symbol: row.fromCurrency.symbol, coin: row.fromCurrency.name, amount: row.fromAmount, sign: '-', type: 2, category: 'Conversion',
            id: row._id, otherCoin: row.toCurrency.name, otherAmount: row.convertedAmount, rate: row.conversionRate, date: row.createdAt
          })
        }
      })
